...
function createView (targetPath = defaultPath) {
  let componentName = ''
  let vueTemplate
  const userProjectDirectory = process.cwd()
  log(`请输入要生成的页面组件名称、会生成在 ${resolve(userProjectDirectory, targetPath)}目录下`)
  ...
  fs.exists(userTemplate, function(exists) {
    if (exists) {
      ...
      if (userTemplateObj.vueTemplate) {
        vueTemplate = userTemplateObj.vueTemplate
      } else {
        errorLog('配置文件应该含有vueTemplate属性')
        process.exit()
      }
    } else {
  ...
  process.stdin.on('data', async chunk => {
     ...
     * Vue页面组件路径
     */
    let componentVueName = resolve(userProjectDirectory, targetPath, inputName)
    // 如果不是以 .vue 结尾的话，自动加上
